...
void TestQgsRasterGPU::testTileUploader()
{
  ...
  // Test tile upload via getTile (which caches)
  const auto gpuTile = uploader.getTile( 0, 0, 0, 1, 1 );
  ...
  // Test tile caching - second call should return same texture
  const auto cachedTile = uploader.getTile( 0, 0, 0, 1, 2 );
  ...
  // Test tile extent calculation (only for tiled rasters)
  const auto tileInfo = uploader.tileInfo( 0 );
  if ( tileInfo.isTiled )
  {
  ...
  {
    qDebug() << "  ⊘ Tile extent test skipped (non-tiled raster)";
  }
